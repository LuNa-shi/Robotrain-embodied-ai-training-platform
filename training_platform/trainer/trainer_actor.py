...
from training_platform.common.minio_utils import get_minio_client, upload_model_to_minio, download_file_from_minio
    ...
    async def train(self, start_epoch: int, end_epoch: int) -> int:
        ...
        if start_epoch == 0:
            ...
            await download_file_from_minio(
                ...
                object_name=f"{self.task.uuid}.zip",
        ...
        if end_epoch >= total_epochs:
            ...
            await upload_model_to_minio(
                client=minio_client,
                dataset_file_local_path=local_model_path,
