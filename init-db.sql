...
CREATE TABLE IF NOT EXISTS dataset_type (
    id SERIAL PRIMARY KEY,              -- 数据集类型ID，自增主键
    type_name VARCHAR(50) UNIQUE NOT NULL, -- 数据集类型名称，唯一且不能为空
    description TEXT DEFAULT NULL       -- 数据集类型描述，可为空
);

CREATE TABLE IF NOT EXISTS dataset (
    id SERIAL PRIMARY KEY,              -- 数据集ID，自增主键
    dataset_name VARCHAR(100) NOT NULL,         -- 数据集名称，不能为空
    dataset_type_id INTEGER NOT NULL REFERENCES dataset_type(id) ON DELETE CASCADE, -- 数据集类型ID，外键引用dataset_types表
...
CREATE TABLE IF NOT EXISTS model_type (
    ...
    dataset_type_id INTEGER NOT NULL REFERENCES dataset_type(id) ON DELETE CASCADE, -- 可使用的数据集类型ID，外键引用dataset_types表
    description TEXT DEFAULT NULL         -- 模型类型描述，可为空
);

CREATE TABLE IF NOT EXISTS model (
    id SERIAL PRIMARY KEY,              -- 模型ID，自增主键
    model_name VARCHAR(100) NOT NULL,         -- 模型名称，不能为空
    description TEXT DEFAULT NULL,       -- 模型描述，可为空
...
CREATE TABLE IF NOT EXISTS train_task(
    ...
    CONSTRAINT chk_completed_fields CHECK (
        CASE
            WHEN status = 'completed' THEN (model_id IS NOT NULL AND logs_uuid IS NOT NULL)
            ELSE TRUE
        END
    )
);
CREATE INDEX IF NOT EXISTS idx_train_tasks_owner_id ON train_task (owner_id);
CREATE INDEX IF NOT EXISTS idx_train_tasks_dataset_id ON train_task (dataset_id);
